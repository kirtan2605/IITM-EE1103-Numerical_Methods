...
double temp=0.0;    //double to turn negative angles into positive
//double error_theta_phi=0.0;       //double to store error in angle if we want to find it.
double error_m=0.0;
...
*/
        ...
        error_m = error_m + (pow(error_mx,2)+pow(error_my,2)+pow(error_mz,2));
    ...
    //PRINTING ERROR FOR GIVEN STEP SIZE
    //printf("%le     %le\n",step_size,error_theta_phi);        //If needed to find error in angle
    //printf("%le     %le\n",step_size,error_m);        //If needed to find the squared error in <m>
    printf("%le     %le\n",(step_size/pow(10,-15)),sqrt(error_m/size));       //prints RMS Error in <m>
    //step size in femto seconds
